...
export function Layout() {
  ...
  const canManageFeatureFlags = hasPermission('feature-flag:admin');
  const canManageAuthorities = hasPermission('authority:admin');
  ...
  const navigation = useMemo(() => {
    ...
    // Insert Feature Flags admin before Jira Settings (permission-gated)
    ...
    // Insert Authorities admin before Jira Settings (permission-gated)
    ...
    // Insert Users admin before Jira Settings (admin-gated)
    if (canManageAuthorities) {
      const jiraIdx0 = nav.findIndex((item) => item.href === '/admin/jira-settings');
      nav.splice(jiraIdx0, 0, {
        name: 'Users',
        href: '/admin/users',
        icon: UsersAdminIcon,
      });
    ...
    return nav;
  }, [orgCapacityEnabled, jobProfilesEnabled, flowForecastEnabled, canManageFeatureFlags, canManageAuthorities]);
